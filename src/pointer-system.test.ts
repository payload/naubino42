...
describe("PointerSystem", function () {
    ...
    describe("find_naub", function () {
        ...
        beforeEach(function () {
            ...
            naub_a = naubino.create_naub({ x: naub_b.radius * -10, y: 0 })
            naub_c = naubino.create_naub({ x: naub_b.radius * 10, y: 0 })
        })
        it("finds naub at naub center", function () {
            const naub = system.find_naub(naub_a.pos)
            assert.equal(naub, naub_a)
        })
        it("finds naub near border, inside", function () {
            const naub = system.find_naub({ x: naub_a.pos.x + naub_a.radius * 0.9, y: naub_a.pos.y })
            assert.equal(naub, naub_a)
        })
        it("finds naub near border, outside", function () {
            const naub = system.find_naub({ x: naub_a.pos.x + naub_a.radius * 1.24, y: naub_a.pos.y })
            assert.equal(naub, naub_a)
        })
        it("finds nothing at nothing", function () {
            const naub = system.find_naub({ x: naub_a.pos.x, y: naub_a.pos.y + naub_a.radius * 10 })
            assert.isNull(naub)
        })
    })
